...
public sealed class GetLoadErrorsQueryHandler(
    ILoadQueryStore queryStore)
{
    private const int MaxPageSize = 100;

    public Task<PagedResult<LoadErrorItem>?> HandleAsync(
        GetLoadErrorsQuery query,
        CancellationToken cancellationToken = default)
    {
        ...
        return queryStore.GetErrorsAsync(
            query.LoadId,
            page,
            pageSize,
            cancellationToken);
